...
var url = __webpack_require__(2983);
// EXTERNAL MODULE: ./src/sources/types.ts
var types = __webpack_require__(1433);
...
class Blogtruyen extends types/* ZenSource */.bp {
    ...
    async getChapterDetails(mangaId, chapterId) {
        ...
        for (let obj of $("#content > img").toArray()){
            ...
            let link = (0,url/* proxyImage */.qG)(obj.attribs["src"], DOMAIN);
            pages.push(link);
        }
    ...
    async getHomePageSections(sectionCallback) {
        ...
        data = await this.requestManager.schedule(request, 1);
        $ = this.cheerio.load(data.data);
        for (let obj1 of $(".row", ".list-mainpage .storyitem").toArray().splice(0, 20)){
            let title = $(`h3.title > a`, obj1).attr("title") ?? "";
            let subtitle = $(`div:nth-child(2) > div:nth-child(4) > span:nth-child(1) > .color-red`, obj1).text();
            const image = $(`div:nth-child(1) > a > img`, obj1).attr("src");
            let id = ($(`div:nth-child(1) > a`, obj1).attr("href") ?? title) ?? "";
        ...
        data = await this.requestManager.schedule(request, 1);
        $ = this.cheerio.load(data.data);
        for (let obj2 of $("a", "#top-newest-story").toArray()){
            let title = $(obj2).attr("title")?.trim() ?? "";
            // let subtitle = $(`.info-bottom > span`, obj).text().split(":")[0].trim();
            const image = $(`img`, obj2).attr("src");
            let id = $(obj2).attr("href") ?? title;
            // if (!id || !subtitle) continue;
        ...
        data = await this.requestManager.schedule(request, 1);
        $ = this.cheerio.load(data.data);
        for (let obj3 of $("p:not(:first-child)", ".list").toArray()){
            let title = $(`a`, obj3).text().trim();
            let subtitle = "Ch\u01B0\u01A1ng " + $(`span:nth-child(2)`, obj3).text().trim();
            const image = $("img", $(obj3).next()).attr("src") ?? "";
            let id = $(`a`, obj3).attr("href") ?? title;
        ...
        data = await this.requestManager.schedule(request, 1);
        $ = this.cheerio.load(data.data);
        for (let obj4 of $("p:not(:first-child)", ".list").toArray()){
            let title = $(`a`, obj4).text().trim();
            let subtitle = "Ch\u01B0\u01A1ng " + $(`span:nth-child(2)`, obj4).text().trim();
            const image = $("img", $(obj4).next()).attr("src") ?? "";
            let id = $(`a`, obj4).attr("href") ?? title;
        ...
        data = await this.requestManager.schedule(request, 1);
        $ = this.cheerio.load(data.data);
        for (let obj5 of $("p:not(:first-child)", ".list").toArray()){
            let title = $(`a`, obj5).text().trim();
            let subtitle = "Ch\u01B0\u01A1ng " + $(`span:nth-child(2)`, obj5).text().trim();
            const image = $("img", $(obj5).next()).attr("src") ?? "";
            let id = $(`a`, obj5).attr("href") ?? title;
...
/* harmony import */ var _MangaDexSettings__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(4599);
/* harmony import */ var _MangaDexHelper__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(3146);
/* harmony import */ var _MangaDexSimilarManga__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(1894);
/* harmony import */ var _external_tag_json__WEBPACK_IMPORTED_MODULE_5__ = __webpack_require__(1171);
/* harmony import */ var _MangaDexParser__WEBPACK_IMPORTED_MODULE_6__ = __webpack_require__(9543);
/* harmony import */ var _types__WEBPACK_IMPORTED_MODULE_7__ = __webpack_require__(1433);
            ...
            if (enabled_homepage_sections.includes(section1.section.id)) {
                ...
                promises.push(this.requestManager.schedule(section1.request, 1).then(async (response)=>{
                    ...
                    switch(section1.section.id){
                        case "latest_updates":
                            {
                                const coversMapping = await this.getCoversMapping(json.data.map((x1)=>{
                                    return x1.relationships.filter((x)=>x.type == "manga"
                                    ).map((x)=>x.id
                                    )[0];
                                }), ratings);
...
/* harmony export */ });
/* harmony import */ var _MangaDexHelper__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(3146);
...
/* harmony import */ var _MangaDexHelper__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(3146);
/* harmony import */ var _MangaDexSimilarManga__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(1894);


const getLanguages = async (stateManager)=>{
    return await stateManager.retrieve("languages") ?? _MangaDexHelper__WEBPACK_IMPORTED_MODULE_0__/* .MDLanguages.getDefault */ .F0.getDefault();
};
const getRatings = async (stateManager)=>{
    return await stateManager.retrieve("ratings") ?? _MangaDexHelper__WEBPACK_IMPORTED_MODULE_0__/* .MDRatings.getDefault */ .HO.getDefault();
};
...
const getHomepageThumbnail = async (stateManager)=>{
    return await stateManager.retrieve("homepage_thumbnail") ?? _MangaDexHelper__WEBPACK_IMPORTED_MODULE_0__/* .MDImageQuality.getDefault */ .hR.getDefault("homepage");
};
const getSearchThumbnail = async (stateManager)=>{
    return await stateManager.retrieve("search_thumbnail") ?? _MangaDexHelper__WEBPACK_IMPORTED_MODULE_0__/* .MDImageQuality.getDefault */ .hR.getDefault("search");
};
const getMangaThumbnail = async (stateManager)=>{
    return await stateManager.retrieve("manga_thumbnail") ?? _MangaDexHelper__WEBPACK_IMPORTED_MODULE_0__/* .MDImageQuality.getDefault */ .hR.getDefault("manga");
};
...
/* harmony export */ });
/* harmony import */ var _MangaDexSettings__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(4599);
...
class NHentai extends types/* ZenSource */.bp {
    ...
    name = "NhenTai";
    icon = "Nhentai.png";
    description = "Nh\u1EC7n t\xe1i tu\u1ED5i th\u01A1.";
    contentRating = external_paperback_extensions_common_.ContentRating.ADULT;
...
class NetTruyen extends types/* ZenSource */.bp {
    ...
    async getChapterDetails(mangaId, chapterId) {
        ...
        return createChapterDetails({
            pages: pages.map((p)=>(0,url/* proxyImage */.qG)(p, DOMAIN)
            ),
            longStrip: false,
            id: chapterId,
            mangaId: mangaId
...
/* harmony import */ var _utils_url__WEBPACK_IMPORTED_MODULE_7__ = __webpack_require__(2983);
/* harmony import */ var _Blogtruyen_Blogtruyen__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(3022);
/* harmony import */ var _MangaDex_MangaDex__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(5321);
/* harmony import */ var _NHentai_NHentai__WEBPACK_IMPORTED_MODULE_5__ = __webpack_require__(4912);
/* harmony import */ var _NetTruyen_NetTruyen__WEBPACK_IMPORTED_MODULE_6__ = __webpack_require__(6733);
...
_global.createRequestManager = (info)=>{
    ...
    instance.interceptors.request.use((config)=>{
        info.interceptor?.interceptRequest(config).then((req)=>{
    ...
    const result = {
        ...info,
        schedule: async (request, retryCount)=>{
...
/* harmony export */ });
/* unused harmony exports ContentRating, createZenMangaTile, createZenManga, createZenChapter */
/* harmony import */ var paperback_extensions_common__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(4580);
/* harmony import */ var paperback_extensions_common__WEBPACK_IMPORTED_MODULE_0___default = /*#__PURE__*/__webpack_require__.n(paperback_extensions_common__WEBPACK_IMPORTED_MODULE_0__);
...
class ZenSource extends paperback_extensions_common__WEBPACK_IMPORTED_MODULE_0__.Source {
    ...
    getChapterInternalURL = (mangaId, chapterId)=>{
